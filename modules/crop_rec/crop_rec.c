...
static void FAST PATH_SelectPathDriveMode_hook(uint32_t* regs, uint32_t* stack, uint32_t pc)
{
    ...
    if (PathDriveMode->zoom == 5)
    {
        ...
        {
            struct patch shift_patch = {
                .addr = (uint8_t *)ShiftAddress,
                .old_value = DefaultShift,
                .new_value = NewShiftVal,
                .size = 4,
                .description = "Center",
                .is_instruction = 0
            };
            int shift_err = apply_patches(&shift_patch, 1);
            Center_Preview_ON = 1;
        }

        if (Clear_Artifacts && !Clear_Artifacts_ON)
        {
            struct patch clear_patch = {
                .addr = (uint8_t *)ClearAddress,
                .old_value = DefaultClear,
                .new_value = NewClearVal,
                .size = 4,
                .description = "Clear",
                .is_instruction = 0
            };
            int clear_err = apply_patches(&clear_patch, 1);
